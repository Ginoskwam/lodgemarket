...
export default function MesAnnoncesPage() {
  ...
  const t = useTranslations('myAnnouncements')
  ...
  const categoriesFr = Object.keys(categoryMapping)
  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (!currentUser) {
        router.push(`/${locale}/auth/login?redirect=/${locale}/annonces/mes-annonces`)
        return
      }
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        <div className="mb-8">
          <h1 className="heading-1 mb-3">{t('title')}</h1>
        ...
        {annonces.length === 0 ? (
          <div className="card p-12 text-center">
            ...
            <h2 className="heading-3 mb-2">{t('noAnnouncements')}</h2>
            <p className="body-text text-gray-secondary mb-8">
              {t('noAnnouncementsDescription')}
            </p>
            <Link href={`/${locale}/annonces/nouvelle`} className="btn-primary inline-block">
              {t('publishFirst')}
            </Link>
          </div>
        ) : allAnnonces.length === 0 ? (
              ...
              >
                ...
                <div className={`flex flex-col flex-1 ${viewSize === 'small' ? 'p-2' : viewSize === 'medium' ? 'p-3' : 'p-4'}`}>
                  <div className="flex-1">
                    ...
                    <div className={`flex items-center gap-1.5 flex-wrap ${
                      ...
                      <span className={`badge-primary ${
                        ...
                        {translateCategory(annonce.categorie).split(' & ')[0]}
                      </span>
                      {viewSize !== 'small' && (
                        <>
                  ...
                  <div className={`flex gap-1 mt-auto ${viewSize === 'small' ? 'flex-col gap-1' : ''}`} onClick={(e) => e.stopPropagation()}>
                    <Link
                      href={`/${locale}/annonces/${annonce.id}/modifier`}
